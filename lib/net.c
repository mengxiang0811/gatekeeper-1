...
gatekeeper_init_network(struct net_config *net_conf)
{
	 ...
	 * are either 0 or 1.
	 */
	while (rsk_sum == 0 || rsk_sum == 0xFF * sizeof(default_rss_key)) {
		rsk_sum = 0;
		while ((ret = syscall(SYS_getrandom, default_rss_key,
			sizeof(default_rss_key), net_conf->random_flags)) !=
			sizeof(default_rss_key));

		for (i = 0; (uint32_t)i < sizeof(default_rss_key); i++)
			rsk_sum += default_rss_key[i];
	}
