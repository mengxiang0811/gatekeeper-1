...
xmit_icmp_ping_reply(struct gatekeeper_if *iface, struct rte_mbuf *pkt)
{
	...
	case ICMP_ECHO_REQUEST_TYPE: {
		 ...
		 * recomputed.
		 */
		uint32_t ip_addr_tmp = icmp_ipv4->src_addr;
	...
	case ICMP_MASK_REQUEST_TYPE: {
		...
		minimum_pkt_size = sizeof(*icmp_eth) + iface->l2_len_out +
			ipv4_hdr_len(icmp_ipv4) + sizeof(*icmph) +
			sizeof(struct in_addr);
		if (pkt->pkt_len < minimum_pkt_size) {
			char *data = rte_pktmbuf_append(pkt, minimum_pkt_size -
					pkt->pkt_len);
			if (data == NULL) {
				LLS_LOG(WARNING,
...
{
	unsigned int num_ping_reply_pkts = 0;
