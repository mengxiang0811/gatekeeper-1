...
gk_hold_arp_and_nd(struct gk_fib *fib, unsigned int lcore_id)
{
	...
	/*
	 * Fib entry with action GK_FWD_GATEWAY should
	...
	if (fib->action == GK_FWD_GATEWAY) {
		eth_cache = &fib->u.nexthop.eth_cache;
		ipv4 = &fib->u.nexthop.ip_addr.ip.v4;
		ipv6 = &fib->u.nexthop.ip_addr.ip.v6;
	} else if (fib->action == GK_FWD_GRANTOR) {
		/* The @next_fib indicates it's an gateway or an neighbor. */
		if (fib->u.grantor.next_fib->action == GK_FWD_GATEWAY) {
			eth_cache = &fib->u.grantor.
				next_fib->u.nexthop.eth_cache;
			ipv4 = &fib->u.grantor.
				next_fib->u.nexthop.ip_addr.ip.v4;
			ipv6 = &fib->u.grantor.
				next_fib->u.nexthop.ip_addr.ip.v6;
