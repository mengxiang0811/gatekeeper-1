	...
	struct gk_instance *instance, struct gk_config *gk_conf)
{
	...
	while (done_lookups < num_ip_flows) {
		...
		ret = rte_hash_lookup_bulk(instance->ip_flow_hash_table,
			(const void **)&flow_arr[done_lookups], num_keys,
			&pos_arr[done_lookups]);
		if (ret != 0) {
			GK_LOG(NOTICE,
	...
	for (i = 0; i < num_ip_flows; i++) {
		...
		ret = process_flow_entry(fe_arr[i],
			&pkt_arr[i], gk_conf, stats);
		if (ret < 0)
